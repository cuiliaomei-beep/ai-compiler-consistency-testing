# Kaggle script kernel (GPU, T4): 8.12 cudagraph stale-output detection + Triton-path edge sweeps with dynamic=True
# and max-autotune (env TORCHINDUCTOR_MAX_AUTOTUNE=1), torch 2.14.0+cu.  Order = value; each sweep resumes.
...
# Triton path, dynamic=True, edge values
for attempt in range(40):
    ...
    if "SWEEP DONE" in r.stdout:
        break
# Triton path, max-autotune (no cudagraphs), edge values
env = dict(os.environ, TORCHINDUCTOR_MAX_AUTOTUNE="1")
for attempt in range(40):
    r = subprocess.run([P, "scripts/opinfo_edge_sweep.py", "--device", "cuda", "--dtypes", "float32,float16", "--max-samples", "2", "--out", "results/edge_cuda_maxautotune.jsonl"], capture_output=True, text=True, env=env)
    print(r.stdout[-3000:], flush=True); print("exit", r.returncode, r.stderr[-400:], flush=True)
    if "SWEEP DONE" in r.stdout:
        break
# keep only results so the pull stays small
for d in os.listdir("."):
    if d != "results":
