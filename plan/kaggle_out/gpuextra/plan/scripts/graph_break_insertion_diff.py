...
def run_variant(programs, backend, log, count_prints=False):
...
def main():
    ...
    a = ap.parse_args()
    out_dir = os.path.join("results", "graph_break_insertion", a.backend if a.relation == "graph_break" else f"{a.relation}_{a.backend}")
    os.makedirs(out_dir, exist_ok=True)
    original = dict(ds.PROGRAMS)
    print(f"torch {torch.__version__}; {len(original)} programs; backend {a.backend}", flush=True)
    quiet = lambda *s: None  # noqa: E731
    base = run_variant(original, a.backend, quiet, count_prints=a.relation == "print_marker")
    ...
    for deep in (False, True):
        ...
        div = run_variant(rw, a.backend, quiet, count_prints=a.relation == "print_marker")
