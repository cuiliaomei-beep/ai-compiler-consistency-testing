    ...
    for n, op in enumerate(ops, 1):
        ...
        try:
            if not op.supports_dtype(dtype, "cpu"):
                ...
                samples = list(itertools.islice(op.sample_inputs("cpu", dtype, requires_grad=False), a.samples))
                fn = op.get_op()
                for si, s in enumerate(samples):
                    ...
                    for pname, (res, err) in paths.items():
                        ...
                        if err:
                            if not eager_err and not any(err.endswith(k) for k in INFRA_ERRS) and not err.startswith("export:"):
