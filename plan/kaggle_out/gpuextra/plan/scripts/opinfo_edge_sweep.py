    ...
    try:
        if UNSUPPORTED:
            ...
            def cast(x):
                ...
                return x
            raw = list(itertools.islice(op.sample_inputs(DEVICE, src, requires_grad=False), max_samples))
            samples = [type(s0)(cast(s0.input), args=cast(s0.args), kwargs=cast(s0.kwargs)) for s0 in raw]
        else:
            samples = list(itertools.islice(op.sample_inputs(DEVICE, dtype, requires_grad=GRAD), max_samples))
