...
def run_one(op, dtype, max_samples, out_path):
    ...
    try:
        samples = list(itertools.islice(op.sample_inputs(DEVICE, dtype, requires_grad=False), max_samples))
    ...
    for si, s in enumerate(samples):
        ...
        if eager_err or comp_err:
            ...
            continue
        fe, fc = [t.cpu() for t in flat(eager)], [t.cpu() for t in flat(comp)]
        if len(fe) != len(fc):
...
def main():
    ...
    work = [(op, dt) for op in ops for dt in dtypes if op.supports_dtype(dt, DEVICE if DEVICE == "cpu" else "cuda")]
    work = work[k::n]
    done = done_keys(out)
