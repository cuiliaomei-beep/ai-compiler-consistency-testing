...
def float_edges(dtype):
    ...
    return [0.0, -0.0, 1.0, -1.0, 0.5, -0.5, 2.0, -2.5, float("inf"), float("-inf"), float("nan"), fi.max, -fi.max, fi.min if fi.min > 0 else fi.tiny,
            ...
            math.sqrt(fi.max) * 1.01, fi.max / 2, -math.sqrt(fi.max) * 1.01, math.pi / 2, math.pi, 0.5 - fi.eps, 0.5 + fi.eps]   # near-singularity values: erfinv/atanh at +-1, acosh at sqrt(max), tan at pi/2
