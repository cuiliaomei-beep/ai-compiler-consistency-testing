...
def float_edges(dtype):
    ...
    return [0.0, -0.0, 1.0, -1.0, 0.5, -0.5, 2.0, -2.5, float("inf"), float("-inf"), float("nan"), fi.max, -fi.max, fi.min if fi.min > 0 else fi.tiny,
            fi.tiny, -fi.tiny, fi.eps, 1.0 + fi.eps, 1.0 - fi.eps, -1.0 + fi.eps, -(1.0 - fi.eps), 3.0, 65504.0, -65504.0, 1e4, -1e4, 0.1, 1e-3, 255.0, 256.0, 1e30 if fi.max > 1e30 else fi.max / 2,
            math.sqrt(fi.max) * 1.01, fi.max / 2, -math.sqrt(fi.max) * 1.01, math.pi / 2, math.pi, 0.5 - fi.eps, 0.5 + fi.eps,
            below_one(dtype), -below_one(dtype)]   # near-singularity values: erfinv/atanh at +-1 (largest value below 1, where 1 - eps is not close enough), acosh at sqrt(max), tan at pi/2


def below_one(dtype):
    return float(torch.nextafter(torch.tensor(1.0, dtype=dtype), torch.tensor(0.0, dtype=dtype)))
...
def edge_fill(t, dtype):
    ...
    new = torch.tensor(seq, dtype=dtype, device=t.device).reshape(t.shape)
    if t.is_contiguous():
        return new
    out = torch.empty_strided(t.shape, t.stride(), dtype=dtype, device=t.device)
    out.copy_(new)
    return out
...
def replace(sample, dtype):
    def rep(x):
        if isinstance(x, torch.Tensor) and x.dtype == dtype:
            t = edge_fill(x.detach().clone(), dtype)
            return t.requires_grad_(True) if x.requires_grad else t
...
DEVICE = "cpu"
GRAD = False       # --grad: OpInfo samples with requires_grad, gradients of sum(outputs) compared as extra tensors
DYNAMIC = False    # --dynamic: torch.compile(dynamic=True)
UNSUPPORTED = False   # --unsupported: run each op on the dtypes it does NOT support (eager raises) and see whether compiled returns


def with_grads(out, inp, args, kwargs):
    """(outputs..., grads of the float outputs w.r.t. every requires_grad leaf...)"""
    outs = flat(out)
    leaves = [t for t in flat((inp, args, kwargs)) if isinstance(t, torch.Tensor) and t.requires_grad]
    diff_outs = [o for o in outs if o.requires_grad and o.dtype.is_floating_point]
    if not leaves or not diff_outs:
        return outs
    loss = sum(o.double().sum() for o in diff_outs)
    grads = torch.autograd.grad(loss, leaves, allow_unused=True)
    return outs + [g if g is not None else torch.zeros(0) for g in grads]


def sticky(e):
    """A device-side assert leaves the CUDA context unusable; the process must restart (the marker records the pair)."""
    return "AcceleratorError" in type(e).__name__ or "CUDA error" in str(e) or "device-side assert" in str(e)
...
def run_one(op, dtype, max_samples, out_path):
    ...
    try:
        if UNSUPPORTED:
            src = next((d for d in (torch.float32, torch.float64, torch.int64, torch.bool, torch.complex64) if op.supports_dtype(d, DEVICE if DEVICE == "cpu" else "cuda")), None)
            if src is None:
                rec["verdict"] = "NO_SAMPLES:no_supported_dtype"; return rec
            def cast(x):
                if isinstance(x, torch.Tensor) and x.dtype == src:
                    return x.to(dtype)
                if isinstance(x, (list, tuple)):
                    return type(x)(cast(y) for y in x)
                if isinstance(x, dict):
                    return {k: cast(v) for k, v in x.items()}
                return x
            raw = take_samples(op.sample_inputs(DEVICE, src, requires_grad=False), max_samples)
            samples = [type(s0)(cast(s0.input), args=cast(s0.args), kwargs=cast(s0.kwargs)) for s0 in raw]
        else:
            samples = take_samples(op.sample_inputs(DEVICE, dtype, requires_grad=GRAD), max_samples)
    except Exception as e:  # noqa: BLE001
        if sticky(e):
            print("sticky CUDA error while sampling", op.name, dtype, "-> restart", flush=True); os._exit(3)
        rec["verdict"] = "NO_SAMPLES:" + type(e).__name__
        return rec
    fn = op.get_op()
    for si, s in enumerate(samples):
        try:
            inp, args, kwargs = (s.input, s.args, s.kwargs) if UNSUPPORTED else replace(s, dtype)
        except Exception as e:  # noqa: BLE001
            rec["errors"].append(f"prep {type(e).__name__}"); continue
        try:
            eager = fn(inp, *args, **kwargs)
            if GRAD:
                eager = with_grads(eager, inp, args, kwargs)
            eager_err = None
        except Exception as e:  # noqa: BLE001
            if sticky(e):
                print("sticky CUDA error in eager", op.name, dtype, "-> restart", flush=True); os._exit(3)
            eager, eager_err = None, type(e).__name__
        torch._dynamo.reset()
        import gc; gc.collect()  # finalise sample iterators before tracing
        try:
            cf = torch.compile(fn, dynamic=DYNAMIC)
            comp = cf(inp, *args, **kwargs)
            if GRAD:
                comp = with_grads(comp, inp, args, kwargs)
            comp_err = None
        except Exception as e:  # noqa: BLE001
            if sticky(e):
                print("sticky CUDA error in compiled", op.name, dtype, "-> restart", flush=True); os._exit(3)
        ...
        if eager_err or comp_err:
            ...
            continue
        fe, fc = [t.detach().cpu() for t in flat(eager)], [t.detach().cpu() for t in flat(comp)]
        if len(fe) != len(fc):
...
def main():
    ...
    ap.add_argument("--unsupported", action="store_true")
    a = ap.parse_args()
    global DEVICE, GRAD, DYNAMIC, UNSUPPORTED
    DEVICE, GRAD, DYNAMIC, UNSUPPORTED = a.device, a.grad, a.dynamic, a.unsupported
    ...
    work = [(op, dt) for op in ops for dt in dtypes if op.supports_dtype(dt, DEVICE if DEVICE == "cpu" else "cuda") != UNSUPPORTED]
    work = work[k::n]
    done = done_keys(out)
