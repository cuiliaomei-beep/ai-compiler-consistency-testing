...
**当前断点（2026-09-22 22:24 收工；最新，从这里续）**：
...
- **22:24 收工点（关机；明天"继续"从这里接）**：0920 批 01–43 + 评论 41b / 42b 全部提交并登记（FILED.md）。**唯一未完的任务：Kaggle T4 `tcc-gpuextra`**（20:23 推送，`kaggle/jobs/gpuextra.py`，三段串行：① 8.12 `scripts/cudagraph_stale_sweep.py`（reduce-overhead 陈旧输出 19 种模式，结果 `results/cudagraph_stale/cases.jsonl`）→ ② `opinfo_edge_sweep.py --device cuda --dynamic --dtypes float32,float16,int32,bool` → `results/edge_cuda_dynamic.jsonl` → ③ `TORCHINDUCTOR_MAX_AUTOTUNE=1` 的 `opinfo_edge_sweep.py --device cuda --dtypes float32,float16 --max-samples 2` → `results/edge_cuda_maxautotune.jsonl`；job 末尾删掉非 results/ 文件，取回快）。关机时状态 RUNNING（已跑约 2 小时）。**明天的第一步**：`cd plan/kaggle && python drive.py status gpuextra`；COMPLETE 则 `python drive.py pull gpuextra`，结果在 `kaggle_out/gpuextra/results/`；仍 RUNNING 则每 20 分钟查一次（最多等到它跑满 9 小时上限）；若 ERROR / 被杀，把 ③ 拆成单独 job 重推（`max-autotune` 最慢）。**分诊顺序**：8.12 看 `stale` 字段——`STALE` 即缺陷（旧输出被静默覆盖）、`ERROR` 是文档保护、`SAME` 安全，`CASE_ERROR` 看 `tb`；②③ 用 `scripts/opinfo_edge_sweep.py --report <jsonl>` 出汇总，与 `kaggle_out/edgegpu/cases.jsonl`（默认模式的 T4 结果，台账 67/81）逐算子比，只看 dynamic / max-autotune **独有**的差异；注意台账 97 的 GC 假阳性（job 用的是修前脚本，grad 相关的怪差异先怀疑它）。分诊后记台账 101（8.12）、102（Triton 两组），有新缺陷按惯例最小化 → 查重 → 起草 44 号。Kaggle 分诊之后的主线是 **NEXT_DIRECTIONS §13：收编成一个工具、一个方法**（接口 → 迁 6 个高产探针 → 其余 → 统一分诊 → 用统一入口重跑验证 73 项全部命中；第 6 步与 harness/ 合并需用户拍板）。其余候选见 §12.5 / §12.6。
- **2026-09-23 19:32**：按用户要求建私有 GitHub 仓库 https://github.com/cuiliaomei-beep/ai-compiler-consistency-testing（PRIVATE，仅所有者，成员由用户自行邀请）。内容从本机复制到独立暂存目录 `D:\wsl\personal\dr	cc-repo`（不在工作目录里建 git）：plan/、paper_tcc/、issues/0913issues+0920issues、harness/、docs/（研究计划、过程、汇报、摘要规范）、ISSUE_OUTCOMES.md，共 15832 个文件、压缩后 24 MB；排除 venv、.obj/.exe/.pyc、zip、paper_tcc/build 与 backups、附件 html；14 个 >10MB 文本结果以 .gz 保存。提交前扫描过密钥（仅命中 report.py 的脱敏正则与测试样例）。git 身份只设在该仓库（GitHub noreply 邮箱）。以后更新：改完工作目录后重跑暂存脚本或手工同步到 tcc-repo，再 commit + push。
- **2026-09-25 18:28**：按 `../FSE2027_实验补充清单_简版_2026-09-25.md` 补实验，总报告 `results/FSE2027_补充实验报告_2026-09-25.md`，台账 102–105。P0-1：冻结 185 程序复算与 09-22 一致（19→25，新增 8 = 2 根因，477 静态插入、544 实际触发、图段 614→1175；第 4 批重名程序覆盖第 3 批一个，故 241）。P0-2：73 项逐项台账 + 35 对历史（19 对修复提交在 2.14 中，15 对仅版本差异；未关闭 issue 检出率 45.7→41.4）；GitHub 只读复查后 ISSUE_OUTCOMES 刷新为 16/13/43/1，B16、C37 待用户定档。P1-1：TorchProbe（temisu）可跑但不适用于 Python 语料，自有种子 222 测试 0 发现，Setting B 5/19→6/19；Random 10 种子均值 10.4；NNSmith 6/6/6。P1-2：52 独立根因三档 36/38/51，误报 0/2，B5 不可复现（CUDA）。P1-3：两臂 3 次计时 13.2 s / 32.7 s。P2 未做。论文未改，待用户定档后统一改。子代理并行（P0-2、P1-1、P1-2），主线程做 P0-1/P1-3。
- **2026-09-23 13:35**：用户指出题目不该加副标题 → 题目改为原句 *Consistency Testing of AI Compiler Stacks by Varying the Compilation*（无副标题、无短题）。全文再对齐：RQ1 改名 "Varying the compilation"（问题句与小节标题）、讨论段标题 "Varying the compilation."、相关工作段标题 "Testing AI compiler stacks"、"varies the compilation path" 改 "varies the compilation"，keywords 去掉 deep learning compilers / compilation path。22 页（18.49+3.32）。备份 `backups/paper_tcc_20260923_1335_v0923h-exact-title.zip`，交付包刷新。
- **2026-09-23 13:31**：用户定题 *Consistency Testing of AI Compiler Stacks by Varying the Compilation: Detecting Source--Artifact Inconsistencies in torch.compile*（短题为主标题）。正文细微处随题目调整：摘要 "AI compiler stacks" / "varies the compilation instead"；引言定义 AI compiler stack、"The compilation itself can be varied"、贡献 1 改为 "Varying the compilation as a dimension of consistency testing"；结论 "by varying the compilation rather than only the program"；相关工作 / 讨论 / 结论中 Python-driven、tracing compilers、DL compiler stacks 统一为 AI compiler stack(s)。多出的几行让讨论溢出到 `\clearpage` 前的空页，去掉 `\clearpage` 会让表 tab-bugs 落到参考文献后（试过，文献段变 4.42 页），改为压缩讨论 10 处措辞，回到 22 页（18.49+3.32）。备份 `backups/paper_tcc_{stamp}_v0923g-title-varying-compilation.zip`，交付包刷新。邮件创新点第 1 条按用户反馈重写。
- **2026-09-23 13:19**：用户要求题目体现方法、用 AI Compiler Stack 代替 Python-Driven、不出现 Compilation Path → 题目改为 *Factor-Controlled Differential Testing of AI Compiler Stacks: Detecting Source--Artifact Inconsistencies in torch.compile*（短题 *Factor-Controlled Differential Testing of AI Compiler Stacks*；keywords 加 "AI compiler stacks"）。重编译 18.49+3.32 页；备份 `backups/paper_tcc_20260923_1319_v0923f-title-ai-compiler-stacks.zip`，交付包 `paper_tcc_2026-09-23.zip` 已刷新。导师邮件改版：创新点每条两三句（标注 AI 凝练），蜕变测试段改为"方法中融合了 MT 思想（3.1 末段），相关工作也讨论"。
- **2026-09-23 13:13**：用户质疑 RQ1 那句话 → 核对台账 89/94/95/100 与 results 日志：graph break 因子当时跑的是 **185** 个程序（不是 274；274 是之后补写 57+33 后的规模），"三轮"实际是三种配置（eager static / dynamic / aot_eager），C44 出自第 4 批里的异常流程序、与 C41/C42 机制无关，第 5 批只划定了 C44 范围没有新缺陷。已改摘要 / 引言 / RQ1 段与答案 / 讨论两段 / 结论；重编译 18.49+3.32 页；备份 `backups/paper_tcc_20260923_1313_v0923e-rq1-corpus-facts.zip`，交付包 `paper_tcc_2026-09-23.zip` 已刷新。
- **2026-09-23 13:07**：题目改为 *Testing along the Compilation Path: Detecting Source--Artifact Consistency Failures in torch.compile*（用户否决 Python-driven）；重编译、交付 zip 刷新，备份 `…_1306_v0923d-title.zip`。GitHub 结果 13:05 刷新：73 = 14 / 10 / 48 / 1，无变化。Kaggle `gpudyn` 仍 RUNNING（未自动轮询，按需查）；`gpuautotune` 待重推。
- **2026-09-23 12:44**：用户决定不再压缩页数、正文不用"第一轮/第二轮"措辞。已改：全文去掉 round 表述（按时间叙述）；两张 issue 表合并为一张 55 行总表（`tab-bugs.tex` 由 `data/findings.csv` 生成，`[p]` 整页）；恢复 fig-rq3、fig-rq1-history；讨论末尾 `\clearpage` 让浮动体在相关工作前排完。正文 18.49 页、参考文献 3.32 页、共 23 页，无未定义引用。备份 `paper_tcc/backups/paper_tcc_*_v0923c-no-rounds-full-tables.zip`，交付件 `toxic_compile/paper_tcc_2026-09-23.zip` 已刷新。
- **2026-09-23 12:36**：文风清理一遍（去掉对仗式短句、"not X but Y"、口语化评价词，摘要 / 引言 / 讨论重写，方法与评估逐句改），正文仍 18.00 页；备份 `paper_tcc/backups/paper_tcc_20260923_1235_v0923b-plain-style.zip`，交付件 `toxic_compile/paper_tcc_2026-09-23.zip` 已刷新。页数说明：ACM TOSEM 无硬性页数上限，17.8–18 页是 09-20 用户自定的目标；若放宽可恢复被删的三张评估图（RQ 消融、历史 issue、成本–产出）和更完整的第二轮描述。
- **2026-09-23 12:14**：**P3 完成**——`paper_tcc` 按 OUTLINE_B 重写（摘要、引言、方法 3、分析器 4、评估 RQ1–RQ6、讨论、相关工作、结论；新表 tab-bugs2、更新 tab-oracles / tab-analyzers / tab-negative、fig-rq5 改为 73 项数据；去掉 fig-rq3 / fig-rq1-history / fig-yield；题目改为 Consistency Testing of Python-Driven Compilers along the Compilation Path）。正文 18.00 页、参考文献 3.34 页（55 条），无未定义引用。数据表 `paper_tcc/data/findings.csv` + `stats.py`。备份：`paper_tcc/backups/paper_tcc_20260923_1213_v0923-path-dimension-rewrite.zip`（含 main.pdf）；交付件 `toxic_compile/paper_tcc_2026-09-23.zip`。基线备份 `..._1155_v0920-baseline-before-rewrite.zip`。P2（RQ 数字）随 findings.csv 完成。待办：P4 工具最低要求（统一入口 + 复现包 + 总表）；Kaggle `gpudyn` 仍在跑，`gpuautotune` 需重推。
- **2026-09-23 11:14**：P0 部分完成——`tcc-gpustale` 取回，8.12 负结果（台账 101）；`gpudyn` 在跑，`gpuautotune` 在 Kaggle 的 kernel 列表里不存在——推送没有生效（可能受并发 GPU 会话数限制），等 `gpudyn` 结束后重推 `python drive.py push gpuautotune`。**统计口径改为四档**（已修复 / 已确认 / Pending / 拒绝，规则在 CLAUDE.md；`issue_outcomes.py` 生成"论文口径"表：73 项 = 14 / 10 / 48 / 1）。用户要求方法中体现蜕变测试、写明创新点。**P1 论证提纲已写：`plan/paper_notes/OUTLINE_2026-09-23.md`，等用户确认三点（题目与表述、创新点顺序、RQ1/RQ2 只用现有数据）**，确认后进入 P2（统计脚本）与 P3（LaTeX）。
- **2026-09-23 10:57**：`tcc-gpuextra` 状态 CANCEL_ACKNOWLEDGED（20:23 推送，估计撞了 Kaggle 的运行上限或被平台取消；`pull` 在后台尝试取残留输出）。已拆成三个独立 T4 job 重推：`tcc-gpustale`（8.12，短）、`tcc-gpudyn`（dynamic 边界值，float32/float16/int32/bool）、`tcc-gpuautotune`（max-autotune，只 float32 × 1 样本）；job 文件 `kaggle/jobs/gpu{stale,dyn,autotune}.py`，已在 `drive.py` 注册。取回：`python drive.py pull <job>`，分诊步骤同 22:24 块。

**2026-09-23 用户重定优先级：论文优先，工具只需能跑、数据可复现。** 计划见 `plan/paper_notes/PLAN_REWRITE_2026-09-23.md`（新主线：一致性失效藏在"程序侧因子 × 编译路径因子"的交点；把一次变一个因子从输入扩展到编译路径；固定 oracle；分层归因；73 项按三类归因统计）。待办改为 P0 Kaggle 分诊 → P1 中文论证提纲（用户确认）→ P2 RQ 统计脚本与发现分类表 → P3 LaTeX 重写 → P4 工具最低要求（统一入口 + 复现包 + 总表）→ P5 其余迁移与 harness 合并（可选）。下面的 T1–T8 只保留作参考。

**待进行任务清单（2026-09-22 22:39 定，按顺序做；完成一项打勾并写日期）**

- [ ] T0 Kaggle `tcc-gpuextra` 取回与分诊（见上方 22:24 块的步骤）→ 台账 101（8.12）、102（Triton dynamic / max-autotune）；有新缺陷 → 最小化 → 查重 → 44 号稿。
- [ ] T1 接口层：`tcc/probe.py` 定义 `InputSource` / `Relation` / `Path` / `Oracle`，oracle 只保留一份实现（合并 `opinfo_edge_sweep.diff`、`oracle.compare`、`dynamo_semantics._norm/close`、`side_effect_diff.norm/close`），输出固定标签 VALUE / DTYPE / NAN / SIGNZERO / STRIDE / ALIAS / RAISES / SILENT / STATE / CRASH；`run.py sweep --input --relation --paths [--device --dynamic --shard --isolate]` 一个入口，接 `sweep_common` 的隔离 / 续跑 / 崩溃标记 / Kaggle 分发。验收：`run.py sweep --input opinfo --relation edge --paths eager,inductor` 复现 `opinfo_edge_sweep.py` 的结果（同一 jsonl 逐条相等）。
- [ ] T2 迁 6 个高产探针为配置：`opinfo_edge_sweep`（含 --grad/--dynamic/--unsupported）、`dispatch_mode_sweep` + `_grad_sweep`、`graph_break_insertion_diff`（graph_break / state_marker / disable_call / print_marker 四种 Relation）、`special_vs_mpmath` + `binary_vs_mpmath`（mpmath 参照作为 Oracle 选项）、`error_parity_sweep`、`export_roundtrip_edge`（export-save-load 与 AOTI 作为 Path，runner 销毁作为 Relation）。每迁一个，用旧脚本的 jsonl 做回归比对。
- [ ] T3 迁其余探针：`layout_alias_sweep`、`scalar_arg_sequence_diff`、`xtarget_numpy_diff`（Numba / JAX Path）、`cxx_intrinsic_algebra_diff`（MSVC Path）、fx 三个（`fx_*`）、`compile_composition_diff`（组合作为 Relation）、`reconstruct_diff`、`cudagraph_stale_sweep`、`autocast_sweep`（autocast 作为 Relation）、`side_effect_diff` / `side_effect_graph_break_diff`、`hop_consistency_diff`、`custom_op_diff`、`int_ub_sweep`、`large_reduce_sweep`、`nightly_decomp_diff`；`diag_*` 与 `$TEMP` 探针归档到 `scripts/_archive/`。
- [ ] T4 统一分诊：`run.py triage` 读全部 jsonl，按 (算子, 标签, 根因关键词) 聚类；从 `TO_SUBMIT.md` + `ISSUE_OUTCOMES.md` 生成已知族表（issue 号 + 关键词 + 代表算子），自动标"已报 #N / 疑似新"；输出 Markdown 汇总。验收：对现有全部 jsonl 跑一遍，已报的 73 项都能被标到对应 issue。
- [ ] T5 回归验证：统一入口重跑全部 (I,T,A/B) 组合（本机 + Kaggle），要求 73 项全部重新命中，产出"探针 → issue"表 `results/probe_issue_map.md`；漏掉的逐条查原因（输入源缺样本 / 关系没迁 / oracle 阈值）。这张表进论文。
- [ ] T6 与 `harness/` 合并——**需用户拍板**：建议以 `plan/tcc` 为基，把 `harness/` 独有的能力（先做 diff 清单）并入；9 月 7 日的旧决定是反向的。
- [ ] T7 论文口径更新：按 `ISSUE_OUTCOMES.md` 改"27 / 11 / 9"；发现按三类分开统计（严格 compile 一致性 / 根因在 eager-fx-运行时 / 跨目标）；方法部分改成 §13.1 的 (I,T,A/B,O) 表述。
- [ ] T8（收编完成后再开的挖掘）：语料第 6 批按缺陷机制定向（重建路径 × graph break 存活对象、副作用回放 × 更多容器方法）；TileLang / Triton 目标；把 `edge_cuda_*` 的模式差分推广到 CPU 的 `cpp_wrapper` / `freezing` 等配置。

- **22:03**：语料第 5 批（33 个"张量错误 × 异常构造"程序，台账 100）→ 22 个程序全是 #198192 族，连 `except Exception` / `contextlib.suppress` 都拦不住 → **42b 评论已贴出**（#198192 comment 5778031868）。graph break × inductor / dynamic 都只是已知族（台账 95）。venv_nightly = 0921。Kaggle `tcc-gpuextra` 仍在跑；本机还有 274 程序 × graph break（eager）在跑。
- **21:46 在跑（等通知）**：本机——`graph_break_insertion_diff.py --backend inductor`（241 程序，`results/graph_break_insertion_inductor.log`）、`--backend eager --dynamic`（新加 `--dynamic` 开关，`…_eager_dynamic.log`）、`venv_nightly` 已升级到 2.15.0.dev20260921+cpu（此后 nightly 复核都用这个版本；之前各稿写的 0911 是当时的版本）——复核结果：14 种 `return_types` 的 `torch.export.save/load` 在 0921 全部正常，`ldl_factor_ex` / `inv_ex` 也已随 PR #197128 修复，不报；Kaggle `tcc-gpuextra` 仍在跑。43 → #198205 已登记。
- **21:39**：7.7 跑完（台账 99）→ **43 号稿（高，Windows AOTI runner 销毁时 OpenMP 线程仍自旋 → 访问违例；Linux 不复现、Windows 独有；**已提交 → #198205**，0920 批 01–43 全部提交）**；`hash_tensor` AOTI proxy executor 报错待最小化（可能 44）。8.11 autocast bf16/fp16 全负（台账 98）。12.6 副作用语料 × graph break 负（台账 96）。Kaggle `tcc-gpuextra`（8.12 + Triton 两组）仍在跑。
- **20:25 在跑（不轮询，等通知）**：本机 4 条——7.7 `scripts/export_roundtrip_edge.py`（OpInfo × 边界值：export→save→load→module 与 aoti_compile_and_package→load 两条取回路径 vs eager，float32，~5 s/算子，`results/export_roundtrip/float32.jsonl` 可续跑）、8.11 `scripts/autocast_sweep.py --acdtype bfloat16`（全 OpInfo，输出 dtype + 数值，`results/autocast/`）、12.6 `scripts/side_effect_graph_break_diff.py`（76 例副作用程序的内层函数逐语句插 graph break，闭包保留；eager 与 aot_eager 两后端，`results/side_effects_gb/`）；Kaggle T4 `tcc-gpuextra`（8.12 `scripts/cudagraph_stale_sweep.py` reduce-overhead 陈旧输出 19 种模式 + Triton 路径 `opinfo_edge_sweep --device cuda --dynamic` 与 `TORCHINDUCTOR_MAX_AUTOTUNE=1` 两组边界值扫描；job 结束时只保留 results/，取回快）。数据集 `tcc-plan` 已含新脚本。42 → #198192、41b 评论已登记。
- **20:12**：语料第 4 批（57 程序，台账 94）基线出 **42 号稿（高）**：张量算子的 `IndexError` 跳过编译函数内的 `except IndexError`（#184340 修了调用方一半，PR #184990 只接了 RuntimeError）；**已提交 → #198192**（0920 批 01–42 全部提交）。41b 评论（41 的触发条件 + 迭代器族）已由用户贴出（#198190 comment 5776320367）。**0920 批全部完成，无待提交项。**241 程序 × graph break（eager / aot_eager / deep 742 处）：新增全是 40/41 族 + lru_cache（已知设计，台账 95）。**本机无任务在跑。**
- **19:52 收工点**：deep 变体重跑完成（565 处，无新增；台账 91 已注）。8.7 的 Windows 独有分歧最小化后 = 自己的 #197102（台账 92 已改）。**0920 批 01–41 全部提交（FILED.md：41 项 GitHub + 3 项微软）；本机与 Kaggle 均无任务在跑、无轮询。** 下一步候选见 NEXT_DIRECTIONS §12.6（语料 × 其它蜕变关系：把函数体拆成两个 compile、给语料补 with/try/生成器程序；对 `side_effect_diff.py` 的 76 例做同样改写）与 §12.5 剩余项（8.12 GPU、Triton/TileLang）。
- **19:49**：`tcc-dynsemlinux` 取回分诊（台账 92：Linux/3.12 与 Windows/3.14 的 Dynamo 语义分歧集合相同，nightly 修了 `python_random`；40/41 的 8 处在 Linux 2.14 + nightly 0922 逐条复现，已写入两稿头表）。本机并行跑完：12.6 重建差分（台账 90，并入 41）、三种附加关系（台账 91，负）、aot_eager 后端（台账 93，负）。**40 → #198189、41 → #198190（用户已提交；41 的"触发条件 + 迭代器族"一段是提交后补的，若要给维护者看需用户另发评论）。** 在跑：deep 变体修正后的 graph_break 重跑（`results/graph_break_insertion_eager_deep2.log`，565 处插入）。
- **19:30**：7.6（`graph_break_insertion_diff.py`，台账 89）→ **40 号稿（高：`OrderedDict.move_to_end` 副作用丢失，LRU 淘汰错）、41 号稿（中：逃出编译区的生成器变 tuple_iterator）**，两稿 checker 通过、2.14 + nightly 输出为真实运行结果；**40 → #198189，41 → #198190（0920 批 01–41 全部提交）**（提交当天再查重；提交后 `mark_filed.py`）。8.7 平台差分：`kaggle/jobs/dynsemlinux.py` 在 Kaggle（Linux，Python 3.12）跑 `run.py dynsem`（2.14 / nightly × static / dynamic / aot_eager）+ `graph_break_insertion_diff.py`，**在跑，不轮询**；取回后 `python scripts/dynsem_platform_compare.py results/dynsem3 kaggle_out/dynsemlinux/plan/results/dynsem_linux_214 …` 比分歧集合（本机基线：dynsem3 19 处 / dynsem3_dynamic 25 处）。数据集 `tcc-plan` 已用 `kaggle/pack.py` 重新打包上传（含 185 程序语料）。
- **19:15**：三条新扫描跑完并记账——8.3b 二元函数/归约 vs mpmath（台账 88：9 命中全是已报族 #198057/#194345/#197895/#197089，无新缺陷）、8.8 编译组合律（台账 86：232 对，21 差异全可解释，负）、8.10 CI 跳过/容差清单（台账 87：Windows 上无一复现，负）。39 → #198183 已登记。**本机与 Kaggle 无任务在跑。** 下一步：7.6 AST 逐语句插 graph break、8.7 Python 版本差分（需装 3.11–3.13 解释器）、GPU 项等 Kaggle 配额。
