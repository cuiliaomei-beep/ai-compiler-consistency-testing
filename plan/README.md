...
## 新角度（NEXT_DIRECTIONS.md §5，2026-09-09 起）

```bash
python run.py decomp --isolate --samples 2 --out results\decomp        # 5.1/5.2 分解-核函数差分 + meta 差分（不编译，15 分钟扫完 OpInfo）
python run.py decomp --isolate --grad --samples 1 --out results\dgrad  #   反向：前向+梯度都走分解
python run.py metamorphic --isolate --samples 1 --out results\meta     # 5.3 编译产物间的变形关系（配置差分/批切分/布局/复用）
python run.py metamorphic --source both --models 150 --out results\mg  #   同一套关系跑在 NNSmith 多算子图 + 语料程序上
python run.py binding --isolate --samples 1 --out results\binding      # 5.4 绑定形式因子（11 种传值方式 × static/dynamic/rebind）
python run.py run --binding-variants 1 --binding-only --out results\bp  #   5.4 接进 Phase 1：每个程序按 8 种绑定形式派生变体后走完整流水线
python run.py diskcache --out results\diskcache                        # 5.6 跨进程磁盘缓存差分（含同源对照）
python run.py aoti --isolate --samples 1 --out results\aoti            # 5.8 export + AOTInductor 路径 vs eager / torch.compile
python scripts\make_model_reproducers.py                               # 5.7 torchvision 小模型 × 5 种配置 → reproducers_models/
python run.py campaign --from-dir reproducers_models --out reports_models
python scripts\confirm_decomp2.py <OpInfo 名> <变体名>                 # 逐条独立复核
```

`--isolate` 把算子分块放进子进程（原生崩溃只损失一个算子），JSONL 增量落盘，同一命令续跑。
结果分诊在 `EXECUTION_REPORT.md` §3 末尾几节，候选在 `TO_SUBMIT.md`，复现与查重在 `reports_decomp/`、`reports_metamorphic/`。
