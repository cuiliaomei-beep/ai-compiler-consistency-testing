...
# pytorch/pytorch — issue 14

| 项 | 值 |
|---|---|
| 提交优先级 | 14 / 27 |
| 原候选编号 | TO_SUBMIT.md 第 4 条；草稿 `plan/issues_0910/04-*.md` |
...
**Summary.** `torch.linalg.vector_norm(x, ord=inf, dim=-1)` on an input with an empty batch dimension does not agree between eager and compile: eager returns an empty `(0,)` result, while every compile backend fails at trace time with "cannot compute the inf norm on the dimension -1 because this dimension is empty"; the same call with `dim=1` compiles (torch 2.14.0 and nightly, CPU and CUDA). A user gets a trace-time error that names a non-empty dimension as empty only when the dim is written negatively. Because the check in `_refs/linalg` tests `d >= 0 and shape[d] != 0` without canonicalising a negative `d`, the outcome is decided by how the dimension is spelled rather than by the tensor.
