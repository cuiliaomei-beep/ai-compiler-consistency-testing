...
# pytorch/pytorch — issue 11

| 项 | 值 |
|---|---|
| 提交优先级 | 11 / 27 |
| 原候选编号 | TO_SUBMIT.md 第 23 条；草稿 `plan/issues_0910/23-*.md` |
...
**Summary.** `torch.compile(optimizer.step)` does not run for SGD with `foreach=True` on complex parameters: eager and `aot_eager` update the parameters, while Inductor raises `AssertionError: <built-in function getitem> is not an OpOverload` in `GraphLowering.call_function` (torch 2.14.0 and nightly, Windows and Linux). A user with complex-valued parameters gets a compile failure only for SGD and only with the default foreach implementation; Adam, AdamW, RMSprop and Adagrad compile the same parameters. Because SGD's foreach path calls `_foreach_add_` on complex tensors directly instead of viewing them as real, the fallback's multi-output result is indexed by `getitem` on something Inductor cannot lower.
